$(document).ready(function () {
    ...
    $("[id='ChangePassword']").click(function () {
        if ($("#password1").val() == "") {
            jAlert("请输密码", "修改密码", function () {
                $("#password1").focus();
            });
            return;
        }
        if ($("#password2").val() == "") {
            jAlert("请输入新密码", "修改密码", function () {
                $("#password2").focus();
            });
            return;
        }
        if ($("#password3").val() == "") {
            jAlert("请输入确认密码", "修改密码", function () {
                $("#password3").focus();
            });
            return;
        }
        if ($("#password3").val() != $("#password2").val()) {
            jAlert("两次输入密码不一致", "修改密码", function () {
                $("#password2").focus();
            });
            return;
        }
        ...
        $.ajax({
            ...
            beforeSend: function () {
                jInformation("正在修改密码...", "修改密码");
            },
            complete: function () {
            },
            success: function (data) {
                if (data.Success == true) {
                    jAlert("修改成功", "修改密码", function () {
                        location.reload();
                    });
                } else {
                    jAlert(data.ResultMsg, "修改密码", function () {
                        location.reload();
                    });
                }
            },
    ...
    $("[id='updateAgent']").click(function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                jInformation("正在更新代理信息...", "更新代理信息");
            },
            complete: function () {
            },
            success: function (data) {
                if (data.Success == true) {
                    jAlert("更新代理信息成功", "更新代理信息", function () {
                        location.reload();
                    });
                } else {
                    jAlert(data.ResultMsg, "更新代理信息", function () {
                        location.reload();
                    });
                }
            },
